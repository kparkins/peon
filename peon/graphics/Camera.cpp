...
void Peon::Camera::SetPosition(const vec3 pos) {
  this->mPosition = pos;
}

vec3 Peon::Camera::GetPosition() const {
...
vec3 Peon::Camera::GetLookDirection() const {
  return normalize(this->mFront);
}
...
mat4 Peon::Camera::GetProjectionMatrix() const {
  return mProjection;
}
...
vec3 Peon::Camera::GetUp() const {
  return this->mUp;
}

void Peon::Camera::SetUp(const vec3& up) {
  this->mUp = up;
}
