...
inline void Peon::RigidBody::SetFriction(float f) {
  mBody->setFriction(f);
}
...
inline void Peon::RigidBody::SetRestitution(float f) {
  mBody->setRestitution(f);
}
...
inline float Peon::RigidBody::GetFriction() const {
...
inline float Peon::RigidBody::GetMass() const {
  return this->mMass;
}
